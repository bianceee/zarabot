...
async function scraping(url) {
    ...
    let data = await page.evaluate((body) => {
    	const product = [];
			const offers = body.querySelectorAll('#sizesContainer li button span');
      const price = body.querySelector('.product-features-prices [data-testid="currentPrice"] .text-title-xl');
      const name = body.querySelector('.product-features .product-name');
      const sku = body.querySelector('.product-reference');
